...
public class LeaveRuleResource {
    ...
    @Timed
    public List<LeaveRule> getAllLeaveRules(@RequestParam(required = false) String filter) {
        if ("leavetype-is-null".equals(filter)) {
            log.debug("REST request to get all LeaveRules where leaveType is null");
            return StreamSupport
                .stream(leaveRuleRepository.findAll().spliterator(), false)
                .filter(leaveRule -> leaveRule.getLeaveType() == null)
